...
export function AnalyzerWidget() {
  ...
  const getTips = (pwd: string, classification: string | undefined) => {
    if (!pwd) return [];
    const tips = [];
      ...
      <AnimatePresence mode="popLayout">
        {result && (
          ...
          >
            ...
            {password && getTips(password, result.classification).length > 0 && (
              <div className="flex flex-col gap-2 mt-2">
                <h4 className="text-sm font-medium text-text mb-1">Tips to Improve</h4>
                <div className="flex flex-col gap-2">
                  {getTips(password, result.classification).map((tip, idx) => (
                    <motion.div
                      key={`tip-${idx}`}
                      initial={{ opacity: 0, x: -10 }}
                      animate={{ opacity: 1, x: 0 }}
